        ...
        for line in f:
            ...
            if len(parts) >= 4:
                try:
                    ...
                    idx = int(parts[1])
                    modello = parts[3]
                    test_completati.add((r, idx, modello))
                except ValueError:
                    pass
    
    print(f"Trovati {len(test_completati)} test già completati. Salto...")
...
for run in range(2, NUM_RUNS+1):
    for i, item_test in enumerate(domande_test, 1):
        ...
        for nome_modello in modelli_da_testare:
            # CONTROLLO RESUME: Se questo test è già nel set, lo salto
            if (run, i, nome_modello) in test_completati:
                # print(f"Salto RUN {run} | Q{i}/{len(domande_test)} | {nome_modello} - Già completato.")
                continue
            ...
            # Tracciatore: CodeCarbon farà automaticamente append al file emissions.csv esistente
