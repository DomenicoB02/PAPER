...
# Rinominati per non sovrascrivere i test buoni già fatti
NOME_FILE_RISPOSTE = "risposte_fix.txt"
NOME_FILE_TEMPERATURE = "temperature_log_fix.csv"
NUM_RUNS = 3
...
# --- TEST VALIDI (COMMENTATI PER RISPARMIARE TEMPO) ---
...
# HellaSwag (Ragionamento di senso comune) - FIX: Aggiunte le opzioni di scelta
...
for item in hellaswag_sample:
    opzioni = item['endings']
    prompt = (
        f"Completa logicamente la seguente situazione scegliendo l'opzione corretta.\n"
        f"Contesto: {item['ctx']}\n"
        f"Opzioni:\n"
        f"A) {opzioni[0]}\n"
        f"B) {opzioni[1]}\n"
        f"C) {opzioni[2]}\n"
        f"D) {opzioni[3]}\n"
        f"Rispondi indicando ESCLUSIVAMENTE la lettera corretta (A, B, C o D)."
    )
...
print(f"Dataset pronto! Totale domande attive per questo run di fix: {len(domande_test)}\n")
...
#─────────────────────────────────────────────
# 3. PREPARAZIONE E SETUP (RESUME AGGIORNATO)
#─────────────────────────────────────────────

task_completati = set()
file_esistono = os.path.exists(NOME_FILE_RISPOSTE)

if not file_esistono:
    print("Inizializzazione nuovi file di log...")
    with open(NOME_FILE_RISPOSTE, "w", encoding="utf-8") as f:
        f.write("### BENCHMARK AI MULTI-DOMINIO (RUN FIX) ###\n")
        f.write("="*80 + "\n\n")

    if not os.path.exists(NOME_FILE_TEMPERATURE):
        with open(NOME_FILE_TEMPERATURE, "w", encoding="utf-8") as f:
            f.write("run,domanda,task,modello,temp_pre,temp_post,cooldown_ok\n")
else:
    print("File delle risposte trovato! Calcolo dei task già completati...")
    with open(NOME_FILE_RISPOSTE, "r", encoding="utf-8") as f:
        for line in f:
            if line.startswith("--- RUN"):
                task_completati.add(line.strip())
    print(f"Trovati {len(task_completati)} test completati con successo. Riprendo da dove si era fermato...\n")

TEMPERATURA_BASELINE = misura_temperatura_baseline()

# Lista completa dei modelli testati nel paper
modelli_da_testare = [
    "llama3.2", 
    "llama3.1", 
    "gemma2:27b", 
...
for run in range(1, NUM_RUNS+1): # Parto da 1 visto che è un file nuovo per i fix
    for i, item_test in enumerate(domande_test, 1):
        
        task_name = item_test["task"]
        domanda = item_test["prompt"]
        
        # LOGICA TEMPERATURA DINAMICA
        if task_name in ["GSM8K", "HumanEval"]:
            temp_corrente = 0.0
            top_p_corrente = 1.0 # Greedy decoding
        else:
            temp_corrente = 0.6
            top_p_corrente = 0.95
        ...
        for nome_modello in modelli_da_testare:
            ...
            # --- CHECK DI RESUME ---
            header_corrente = f"--- RUN {run} | DOMANDA {i} | TASK: {task_name} | MODELLO: {nome_modello} ---"
            
            if header_corrente in task_completati:
                print(f"⏩ Saltato: RUN {run} | Q{i} | {nome_modello} (Già nel file txt)")
                continue
            # -----------------------

            temp_pre = get_gpu_temp()
            print(f"\n▶ Esecuzione: RUN {run} | Q{i}/{len(domande_test)} [{task_name} | Temp: {temp_corrente}] | {nome_modello} | Temp pre: {temp_pre}°C")

            llm = ChatOllama(
                model=nome_modello,
                temperature=temp_corrente,
                num_predict=512,
                num_ctx=2048,
                repeat_penalty=1.2,
                top_p=top_p_corrente,
                top_k=20,
            )
            ...
            # Cambiato file di output in emissions_fix.csv
            tracker = EmissionsTracker(
                project_name=f"Q{i}_{nome_modello}_{task_name}",
                output_dir=".",
                output_file="emissions_fix.csv",
                log_level="error"
            )
